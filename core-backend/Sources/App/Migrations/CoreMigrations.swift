...
struct CreateSinpeTransaction: AsyncMigration {
    func prepare(on database: Database) async throws {
        try await database.schema("sinpe_transactions")
            ...
            .field("payment_method", .string, .required, .sql(.default("sinpe")))
            .field("receipt_url", .string)
            .field("ai_status", .string, .required, .sql(.default("pending")))
            .field("owner_status", .string, .required, .sql(.default("pending")))
