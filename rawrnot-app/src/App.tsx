...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      {/* Global Noise Overlay for Lightcraft */}
      ...
      <header className="fixed top-0 w-full z-[100] flex justify-between items-center px-6 py-4 bg-background/40 backdrop-blur-3xl shadow-[0_20px_40px_rgba(0,0,0,0.4)] border-b border-white/5 transition-colors duration-700">
        ...
        >
          ...
          <span className="font-headline tracking-tighter text-2xl font-bold uppercase text-primary italic font-black">
            RAW'R'NOT
          </span>
        </motion.div>
        
        <nav className="hidden lg:flex gap-4 items-center">
          {(['portfolio', 'blog', 'about', 'quoting', 'briefing', 'crm'] as View[]).map((view) => (
            <button 
              key={view}
          ...
          <button 
            ...
            className="text-[10px] font-black uppercase tracking-tighter text-on-background/40 hover:text-primary px-2 py-1 border border-on-background/10 rounded transition-colors"
          >
            {language === 'en' ? 'ES' : 'EN'}
          </button>
        </nav>
        ...
        <div className="flex items-center gap-4">
          <motion.div 
            ...
            className="w-10 h-10 rounded-full overflow-hidden border border-primary/30 cursor-pointer"
          >
            <img 
              className="w-full h-full object-cover" 
              alt="majestic lion"
              src="https://images.unsplash.com/photo-1546182990-dffeafbe841d?auto=format&fit=crop&q=80&w=200&h=200" 
            />
          </motion.div>
        </div>
      </header>
